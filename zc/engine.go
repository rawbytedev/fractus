...
func EncodeRecordHot(schemaID uint64, hotTags []uint16, fields []db.FieldValue) ([]byte, error) {
    ...
    // Build header
    header := db.BuildHeader(vt, &db.LayoutPlan{HeaderFlags: db.FlagPadding, SchemaID: schemaID, HotTags: hotTags})
...
func GenPayloads(fields []db.FieldValue) ([]byte, []db.OffsetMap) {
    var tmp []byte
    next := 0
    var fieldbuf []byte
    var offmap []db.OffsetMap
    for _, field := range fields {
        if field.CompFlags&db.ArrayMask != 0 {
        ...
        tmp = append(tmp, field.Payload...)
        offmap = append(offmap, db.OffsetMap{tag: field.Tag, compflag: field.CompFlags, offset: uint32(next)})
        next = len(tmp)
    }
    return tmp, offmap
}

func GeneVtables(offsets []db.OffsetMap) []byte {
